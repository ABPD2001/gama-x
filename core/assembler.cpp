...
vector<_uva_snippet_t> UVA_ASSEMBLER::_transpile_(string content, uint16_t line_idx){
	...
	for(uint32_t i = 0; i<lines.size(); i++){
		...
		for(uint8_t i = 0; i<params.size(); i++){
			if(filter(params[i],empties,7).empty()) this->errors.push_back((_uva_error_t) {"'"+CMD+"' instruction set arguemnt "+to_string(i+1) + " has invalid syntax!","INVALID_VALUE_SYNTAX","syntax",lx});
		}
			...
			if(CMD == "mov"){
				...
				else this->errors.push_back((_uva_error_t){"'mov' instruction set value missed or is invalid!","MISSED_OR_INVALID_REGISTER_NAME","syntax",lx});
			}
				...
				if(params.size()>2 || params.size()<1) this->errors.push_back((_uva_error_t){"'jmp' instruction set must have line index to jump!","MISSED_VALUE","syntax",lx});
				...
				else this->errors.push_back((_uva_error_t){"'jmp' instruction set jump address has invalid syntax!","INVALID_ARGUEMNT_SYNTAX","syntax",lx});
			}
			else if(CMD == "cmp"){
				if(params.size()!=2) {
				...
				if(params.size()!=2) {
					...
					else this->errors.push_back((_uva_error_t){"'add' instruction set arguements are invalid!","INVALID_ARGUMENTS","value",lx});
			}
				...
				if(params.size()!=1) {
					...
					else this->errors.push_back((_uva_error_t){"'incr' instructions set argmuents are invalid!","INVALID_ARGUMENTS","value",lx});
			}
				...
				if(params.size()!=2) {
					...
					const signed short int reg_idx = find(vir_regs_names,params[0]);
                                        ...
                                        if(reg_idx > -1) vir_regs[reg_idx] -= reg2;
                                        else this->errors.push_back((_uva_error_t){"'add' instruction set arguments are invalid!","INVALID_ARGUMENTS","value",lx});
                                }
				...
				if(params.size()!=1) {
					this->errors.push_back((_uva_error_t){"'decr' instructions set arguments missed!","MISSED_ARGUMENTS","value",lx});
                                	continue;
                                 	       ...
                                 	       const signed short int reg_idx = find(vir_regs_names,params[0]);
                                        	if(reg_idx > -1) vir_regs[reg_idx]--;
                                        	else this->errors.push_back((_uva_error_t){"'decr' instructions set arguments is invalid!","INVALID_ARGUMENT","value",lx});
                                	}
			}
			else if(CMD == "push"){
				if(params.size() != 1) {
					...
					if(params[0][0] == '#') this->vir_stack.push_back(to_double(trim(params[0])));
						...
						else this->errors.push_back((_uva_error_t){"'push' instruction set register is invalid!","INVALID_STACK_REGISTER","value",lx});
					}
				...
				if(params.size() != 1) {
					...
					else this->errors.push_back((_uva_error_t){"'pull' instruction set argument is invalid!","INVALID_ARGUMENT","value",lx});
				...
				//}
					...
					else this->errors.push_back((_uva_error_t){"'call' instructions set arguments are invalid!","INVALID_VALUE","value",lx}); 
				}
			...
			else this->errors.push_back((_uva_error_t){"Invalid instruction entered!","INVALID_INSTRUCTION","decode",lx});
		}
	}
...
void UVA_ASSEMBLER::_pre_processors_(string &content){
	...
	for(uint32_t i = 0; i<lines.size(); i++){
		...
		if(CMD == ".replace") {
			if(parts.size() != 3 || parts[1].empty() || parts[2].empty()) this->errors.push_back((_uva_error_t){"'.replace' pre-processor set arguments missed!","CO_PROCCESSOR_VALUES_MISSED","syntax",i});			
			else {
			...
			if(parts.size() != 2 || parts[1].empty()) this->errors.push_back((_uva_error_t){"'.main' pre-proccessor set argument missed!","CO_PROCCESSOR_VALUE_MISSED","syntax",i});
			else {
...
vector<_uva_label_t> UVA_ASSEMBLER::_slice_labels_(string content){
	...
	for(uint32_t i = 0; i<lines.size(); i++){
		...
		if(cur_label.empty() && lines[i].find(':') != string::npos && lines.size()-1 != i) {
			...
			label_t label = {content,cur_label,idx_line};
			cur_label.clear();
...
vector<_uva_snippet_t> UVA_ASSEMBLER::compile(){
	...
	string co_processed_input;

	this->_pre_processors_(co_processed_input);
	this->labels = this->_slice_labels_(file_content_result);
	bool found = false;

	for(label_t lbl: this->labels){
		if(lbl.name == this->startLabel) {
			out += this->_transpile_(lbl.text,this->vir_regs_names,this->vir_regs,lbl.line_idx);
			found = true;
			break;
		}
	}
