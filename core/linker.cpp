...
string _GXL_::_merge_pre_processors_(vector<_GXL_file_t> files){
	...
	for(_GXL_file_t f:files){
		...
		const short int idx_module = f.content.find(".module");
	        const short int idx_main = f.content.find(".main");
		...
		const string pre_processors = f.content.substr(0,idx);

		if(idx_module != string::npos) output += pre_processors+'\n';
		else output = pre_processors.replace(idx_module,7,"")+'\n';
	}
