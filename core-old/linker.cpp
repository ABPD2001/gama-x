...
string _GAMA_X_LINKER_::_merge_pre_processors_(vector<_GAMA_X_LINKER_file_t> files) {
	...
	for(uint32_t i = 0; i<files.size(); i++){
		...
		pre_processors_sections.push_back(pre_processors.replace(pre_processors.find(".module"),7,""));
	}
...
string _GAMA_X_LINKER_::_merge_instructions_(vector<_GAMA_X_LINKER_file_t> files){
	...
	for(uint32_t i = 0; i<files.size(); i++){
		...
		string file_instructions = files[i].content.substr(idx+4,files[i].content.length()-(idx+4));
